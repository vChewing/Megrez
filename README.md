...
- `compositor.insertKeyAtCursor(key: "gao1")` 可以在當前的游標位置插入讀音「gao1」。
- `compositor.deleteKeyToTheFrontOfCursor()` 的作用是：朝著往文字輸入方向、砍掉一個與游標相鄰的讀音。反之，`deleteKeyAtTheRearOfCursor` 則朝著與文字輸入方向相反的方向、砍掉一個與游標相鄰的讀音。
